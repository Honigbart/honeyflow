...
- If it does NOT exist (plan was quick-planned but never executed): this is a re-plan. Read any existing `.ai/plans/<slug>/codex_critique.md` for context, then overwrite `final_plan.md`. Delete `codex_critique.md` after overwriting since it critiques the old plan.
...
4. If re-planning an existing slug, read the existing `codex_critique.md` if present and incorporate relevant findings. If the critique contains a provenance note indicating it was generated by Claude as a fallback (not Codex), note this in `## 4. Accepted Critiques` so the distinction is visible downstream.
...
3. If re-planning: delete stale `.ai/plans/<slug>/codex_critique.md` if it existed
4. Ensure `.ai/plans.md` has an entry for this slug with status `active`
5. Then provide the short in-chat summary
