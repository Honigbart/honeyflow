...
- If it does NOT exist (plan was quick-planned but never executed): this is a re-plan. Read any existing `.ai/plans/<slug>/codex_critique.md` and `.ai/plans/<slug>/ollama_critique.md` for context, then overwrite `final_plan.md`. Delete stale critique artifacts after overwriting since they critique the old plan.
...
3. If re-planning: delete stale `.ai/plans/<slug>/codex_critique.md` and `.ai/plans/<slug>/ollama_critique.md` if they existed
