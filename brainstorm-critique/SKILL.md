...
- do not stop or ask for permission — fall back to Claude critique automatically to keep the user's flow unbroken
- write the critique to `.ai/plans/<slug>/codex_critique.md` so the downstream pipeline can continue
- prepend a clear provenance note stating that this file was generated by Claude as a fallback because Codex was unavailable, including the failure reason if known (usage limit, quota, rate limit, auth failure, or process error)
- note briefly in chat that Codex was unavailable and Claude produced the critique instead — one line, not a conversation
